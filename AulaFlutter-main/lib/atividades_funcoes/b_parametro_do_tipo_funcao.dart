...
R:  Foi adicionado a variável escolha e o código foi reorganizado, tornando-o mais legível, além disso foram criadas mais funções caracterizando um baixo acoplamento.

2) O que é e para que serve parâmtros do tipo função? Em que caso utilizamos?
R:  Um parâmetro do tipo função passa a chamda dessa função, um exemplo de caso onde a utilizamos é igual ao código acima, onde dependendo de qual forma queremos calcular a nota
usada no calculo de aprovação, podemos passar diferentes funções como um parâmetro para a função verificarAprovação.
...
*/
void calcularAcrescimo(double salario, double valor){
  double porcentagem = (valor/salario)*100;
  print("O acréscimo do salário foi de "+porcentagem.toString() + "%.");
}

void calcularDecrescimo(double salario, double valor){
  double porcentagem = (valor/salario)*100;
  print("O decréscimo do salário foi de " + porcentagem.toString() + "%.");
}

void calcularBonus(double salario, double valor) {
  double porcentagem = ((valor*0.9)/salario)*100;
  print("O acréscimo do salário foi de" + porcentagem.toString() + "%.");
}

void verificaPorcentagem(double salario, double valor, Function calculo){
  calculo(salario,valor);
}

void main(List<String> arguments){
  print('''
    Escolha uma opção: 
    (1) - verificar porcentagem do acréscimo de salário;
    (2) - verificar porcentagem do decréscimo de salário;
    (3) - verificar porcentagem do aumento de bônus(com encargo de 10%) em relação ao salário.
  ''');
  int escolha = int.parse(stdin.readLineSync()!);
  ...
  if(escolha == 1){
    verificaPorcentagem(salario, valor,calcularAcrescimo);
  }else if(escolha == 2){
    verificaPorcentagem(salario, valor,calcularDecrescimo);
  } else {
    verificaPorcentagem(salario, valor,verificaPorcentagem);
  }
}
...
*/
String calcularIMC(int idade, double altura, double peso) {
...
String calcularMediaAltura(int idade, double altura, double peso) {
  double media = 0;
  ...
  if (altura < media) {
    return "Sua altura está abaixo da média brasileira para sua idade.";
  } else if (altura > media) {
    return "Sua altura está acima da média brasileira para sua idade.";
  } else {
    return "Sua altura está na média brasileira para sua idade.";
  }
}

void printaCalculo(int idade, double altura, double peso, Function function){
  print(function(idade, altura, peso));
}
...
void menuProjeto(){
  ...
  print('''
    Escolha uma opção: 
    (1) - Calcular seu IMC;
    (2) - Comparar sua altura com a média brasileira;
  ''');
  int escolha = int.parse(stdin.readLineSync()!);
  if (escolha == 1){
    printaCalculo(idade, altura, peso, calcularIMC);
  }
   if (escolha == 2){
    printaCalculo(idade, altura, peso, calcularMediaAltura);
  }
}